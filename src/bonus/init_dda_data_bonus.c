...
/*   Updated: 2024/04/20 23:58:27 by minsepar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_side_dist(t_dda *dda, t_user *user)
{
	...
	{
		dda->step_x = -1;
		dda->side_dist_x = (user->x - dda->map_x) * dda->delta_dist_x;
	}
	else
	{
		dda->step_x = 1;
		dda->side_dist_x = (dda->map_x + 1.0 - user->x) * dda->delta_dist_x;
	}
	if (dda->raydir_y < 0)
	{
		dda->step_y = -1;
		dda->side_dist_y = (user->y - dda->map_y) * dda->delta_dist_y;
	}
	else
	{
		dda->step_y = 1;
		dda->side_dist_y = (dda->map_y + 1.0 - user->y) * dda->delta_dist_y;
	}
}
...
void	init_data(t_dda *dda, t_user *user, int x_pixel_num)
{
	...
	dda->map_x = (int) user->x;
	dda->map_y = (int) user->y;
	init_delta_dist(dda);
